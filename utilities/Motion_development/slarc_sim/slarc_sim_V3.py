...
def generate_hexapod_urdf(filename="slarc_primitives.urdf"):
    ...
    # Massenverteilung (realistisch):
    # base_link  = Chassis(0.40) + Elektronik(0.30) + Akku(0.20) + 6×Hüftservo(6×0.065) = 1.29 kg
    # coxa link  = Coxasegment(0.015) + 1×Femurservo(0.065) = 0.080 kg
    # femur link = Femursegment(0.030) + 1×Tibiaservo(0.065) = 0.095 kg
...
def create_staircase():
    ...
    # ── 5 Stufen aufwärts ────────────────────────────────────────────
    step_h    = 0.170   # Stufenhöhe (normale Haustreppe)
    step_d    = 0.250   # Stufentiefe (normale Haustreppe)
    ...
    small_h   = 0.170  # gleiche Höhe wie große Stufen
    small_d   = 0.250  # gleiche Tiefe
    x_down    = x_top + plateau_len         # 3.0 m
    n_down    = 5   # 5 Stufen abwärts (gleiche Höhe wie aufwärts)
    ...
    print(f"  Layout: 5 Stufen aufwärts  (x=1.0–2.25m, +{top_h*100:.0f}cm)")
    print(f"         Plateau (Niveau)    (x=2.25–3.25m)")
...
class StereoCameras:
    ...
    Dual-Kamera mit SLARC-Parametern:
      Baseline  : 50 mm
      Brennweite: 2.8 mm  →  FOV 83.8°h × 67.7°v (Scaling auf 640×480, FOV identisch)
      Auflösung : 640×480 (wie im Training)

    Die Kameraposition folgt dem Roboter-Körper.
    """
    # IMX296: 1456×1088px, 3.45µm Pixelpitch, f=2.8mm
    # fx_full = 2.8 / 0.00345 = 811.6 px bei 1456px Breite
    # Scaling auf 640×480 (kein Crop — FOV bleibt identisch):
    IMG_W   = 640
    IMG_H   = 480
    NEAR    = 0.01   # m
    FAR     = 10.0   # m

    # fx = 811.6 × (640/1456) = 357 px  /  fy = 811.6 × (480/1088) = 358 px
    # FOV nach Scaling: 83.8° H × 67.7° V (gleich wie volle Auflösung ✓)
    FX = 357.0
    FY = 358.0
...
class HexapodIK:
    def __init__(self):
        self.L_C = 0.060; self.L_F = 0.175; self.L_T = 0.150
...
class HexapodLeg:
    def __init__(self, name, m_x, m_y, m_yaw):
        ...
        def_x = 0.22; def_y = 0.00; def_z = -0.19
...
class SlarcController:
    ...
    def update_torque_display(self):
        """
        Zeigt Gelenkdrehmomente als farbige Debug-Linien und Text.
        Grün = sicher (<50%), Gelb = Warnung (50-80%), Rot = kritisch (>80%).
        Nur alle 10 Frames aktualisiert (Performance).
        """
        if not hasattr(self, '_torque_frame'):
            self._torque_frame = 0
            self._torque_lines = []
        self._torque_frame += 1
        if self._torque_frame % 10 != 0:
            return

        TAU_MAX = 2.94   # N·m ST3215 @ 12V
        # Drehmomentschwellen (Anteil von TAU_MAX)
        WARN  = 0.50
        CRIT  = 0.80

        # Alle alten Debug-Items entfernen
        for item_id in self._torque_lines:
            try: p.removeUserDebugItem(item_id)
            except: pass
        self._torque_lines.clear()
        ...
        for i in range(p.getNumJoints(self.robot_id)):
            ...
            # applied motor torque ist state[3]
            tau = abs(state[3])
            ratio = tau / TAU_MAX

            if ratio > max_tau / TAU_MAX:
                max_tau = tau
                max_joint = jname

            # Farbe nach Auslastung
            if ratio < WARN:
                color = [0.1, 0.9, 0.1]    # grün
            elif ratio < CRIT:
                color = [1.0, 0.8, 0.0]    # gelb
            else:
                color = [1.0, 0.1, 0.1]    # rot

            # Gelenkposition
            link_state = p.getLinkState(self.robot_id, i)
            lx, ly, lz = link_state[0]

            # Balkenlänge proportional zur Auslastung (max 0.08m)
            bar_len = max(0.005, ratio * 0.08)
            line_id = p.addUserDebugLine(
                [lx, ly, lz],
                [lx, ly, lz + bar_len],
                color, lineWidth=3, lifeTime=0.12
            )
            self._torque_lines.append(line_id)

        # Gesamtanzeige über dem Roboter
        ratio_max = max_tau / TAU_MAX
        if ratio_max < WARN:   color_txt = [0.1, 0.9, 0.1]
        elif ratio_max < CRIT: color_txt = [1.0, 0.8, 0.0]
        else:                  color_txt = [1.0, 0.1, 0.1]

        txt_id = p.addUserDebugText(
            f"τ_max={max_tau:.2f}Nm ({ratio_max*100:.0f}%)  [{max_joint.replace('_joint','')}]",
            [pos[0], pos[1], pos[2] + 0.35],
            textColorRGB=color_txt, lifeTime=0.12, textSize=1.0
        )
        self._torque_lines.append(txt_id)
...
def main():
    ...
    print(" Drehmomente werden automatisch angezeigt (grün/gelb/rot)")
