...
SLARC_VERSION = "V8.26-footclamp"   # in Log-Metazeile geschrieben -> Lauf eindeutig zuordenbar
...
class SlarcController:
    def __init__(self):
        ...
        self.stairs = StairModel(STAIR_RISE, STAIR_RUN, x_start=1.0, n_up=5, plateau_len=1.0)
        ...
        self.G_REAR_REACH = 0.13   # Hinterbein-Offset nach hinten pro rad Nase-hoch-Pitch [m/rad]
        ...
        self.G_CLIMB_RAISE = 0.08  # beim Klettern: Rumpf um bis zu 8cm anheben (gerampt), damit die
                                   # Körper-Vorderkante über die nächste Stufe kommt statt anzustoßen.
        self._g_climb_raise = 0.0  # gerampter Ist-Wert (kein Höhen-Sprung -> kein Hüpfen)
        ...
        self.HOLD_H_KP   = 0.05    # Regler 3: Höhen-Regler-Verstärkung (Körperhöhe über Boden)
    ...
    def _gait_neutral(self, leg, splay):
        ...
        nx = leg.mount_x + self.G_RADIUS*math.cos(yaw)
        ny = leg.mount_y + self.G_RADIUS*math.sin(yaw)
    ...
    def _step_gait(self):
        ...
        self._g_pitch_meas = pitch_m          # gemessener Körper-Pitch (IMU) für Kletter-Erkennung
        if self.auto_balance:
            self.roll_bal, self.pitch_bal = self.balance.update(
                roll_m, pitch_m, 1/240., pitch_sp=self._g_climb_pitch)
        else:
            self.roll_bal, self.pitch_bal = self.balance.decay()
        self._g_pitch_eff = self.pitch_bal + self.pitch
        self._g_roll_eff  = self.roll_bal + self.roll

        # W/S-Höhe rampen (kein Hüpfen): Sollwert glatt nachführen
        d = _clamp(self._bho_target - self.body_height_offset, -self.G_BHO_RAMP, self.G_BHO_RAMP)
        self.body_height_offset += d
        ...
        climbing = on_stairs   # Vortrieb/Höhen-Einfrierung: Treppe erkannt (unabhängig vom Pitch-Sollwert)

        # P2 Höhe (beim Klettern eingefroren)
        fem = [p.getJointState(self.robot_id, self.joint_map[f"{l.name}_femur_joint"])[0]
               for l in self.legs if self._g_contact[l.name]]
        if fem and not climbing:
            err = self.G_FEMUR_TGT - sum(fem)/len(fem)
            self._g_offset = _clamp(self._g_offset - err*self.G_BODY_H_KP,
                                    -self.G_OFFSET_LIM, self.G_OFFSET_LIM)
        ...
        stance_lost = [n for n in stance_now if not self._g_contact[n]]
        if self.G_FREEZE_ENABLE and quasi_static and moving and stance_lost:
        ...
        seek_v = self.G_SEEK_V_FREEZE if frozen else self.G_SEEK_V   # Freeze: Boden schneller wiederfinden
        for leg in self.legs:
            ...
            if not self._g_contact[leg.name]:
                # KONTAKT-FIX: ein tragendes Bein ohne µ-Switch-Kontakt schwebt über der
                # (evtl. unstetigen) Fläche. Es folgt dem Körper-Anheben dzb NICHT — sonst
                # hebt dzb (Klettern/Entry-Boost) den Fuß über die SEEK hinweg von der Stufe
                # und der Vortrieb schiebt ins Leere. Stattdessen aktiv senken, bis es wieder
                # trägt. Portabel: nur µ-Switch-Zustand + FK-Höhe, keine Weltkoordinaten.
                self._g_base_z[leg.name] -= dzb                # Anheben zurücknehmen
                self._g_base_z[leg.name] = max(self._g_base_z[leg.name] - seek_v,
                                               self.G_Z - 0.13)   # SEEK bis -0.25
            f[2] = _clamp(self._g_base_z[leg.name] + self._g_offset - self.body_height_offset
                          - self._g_climb_raise, self.G_Z_FLOOR, -0.10)
    ...
    def hold_update(self):
        """[HALTE-MODUS] Isoliert von Gehen/Klettern: Rumpf steht (cmd=0, kein Schwung).
        Zwei ENTKOPPELTE Kontaktregler sorgen für sechs ruhig tragende Füße:
          Regler 1 (µ-Switch, PRO BEIN): Schalter offen -> Fuß sanft nachsenken, bis er
                   schließt; geschlossen -> einrasten (kein Weiterdrücken). Kontaktherstellung.
          Regler 2 (IMU-Pitch/Roll, GLOBAL): drückt ein Fuß hoch, kippt der Rumpf aus dem Soll;
                   der Balancer rotiert die Fußebene und verkürzt genau dieses Bein. Lastausgleich.
        Entkopplung: kontaktloses Bein -> Regler 1 (senken), tragendes -> Regler 2 (Last).
        Dazwischen ('berührt sanft, drückt nicht') ruhen beide -> stabiles Totband.
        Alles portabel: µ-Switch + IMU, keine Weltkoordinaten."""
        self._refresh_contact_cache()
        roll_m, pitch_m = self._read_body_attitude()
        self._g_pitch_meas = pitch_m
        # Regler 2 (global): Balancer auf Soll = eben (0). Rotiert die Fußebene in _set_leg.
        if self.auto_balance:
            self.roll_bal, self.pitch_bal = self.balance.update(roll_m, pitch_m, 1/240., pitch_sp=0.0)
        else:
            self.roll_bal, self.pitch_bal = self.balance.decay()
        self._g_pitch_eff = self.pitch_bal
        self._g_roll_eff  = self.roll_bal
        # Regler 3 (Höhe): Körperhöhe über dem BODEN konstant halten. Aus FK die IST-Fuß-z
        # rel Hüfte der tragenden Beine; der MEDIAN ist robust gegen einen einzelnen Bump (die
        # Bodenfüße dominieren, der hochstehende Fuß verzieht ihn nicht). body_height_offset so
        # nachführen, dass der Rumpf NICHT dem Bump folgt, sondern auf Bodenhöhe bleibt. Portabel.
        fz = []
        for leg in self.legs:
            if not self._foot_touch_down(leg):
                continue
            ci = self.joint_map.get(f"{leg.name}_coxa_joint")
            fi = self.joint_map.get(f"{leg.name}_femur_joint")
            ti = self.joint_map.get(f"{leg.name}_tibia_joint")
            if None in (ci, fi, ti):
                continue
            tc = p.getJointState(self.robot_id, ci)[0]
            tf = p.getJointState(self.robot_id, fi)[0]
            tt = p.getJointState(self.robot_id, ti)[0]
            fz.append(self.ik.forward(tc, tf, tt)[2])       # Fuß-z rel Hüfte (FK)
        over_height = 0.0                                   # >0: Rumpf über Soll -> R1 darf retrahieren
        if len(fz) >= 3:
            fz.sort()
            med = fz[len(fz)//2]                            # Median (Bodenhöhe)
            h_ist = -med                                    # Körperhöhe über Boden
            err = h_ist - abs(self.G_Z)                     # Soll = |G_Z|
            self.body_height_offset -= self.HOLD_H_KP * err # zu hoch -> senken, zu tief -> heben
            self.body_height_offset = _clamp(self.body_height_offset, -0.10, 0.15)
            over_height = max(0.0, err - self.HOLD_RETRACT_DB)  # Überhöhe jenseits Totband
        for leg in self.legs:
            name = leg.name
            nx, ny = self._gait_neutral(leg, self.G_HOLD_SPLAY)  # neutrale Standpose (Spider)
            # Regler 1 (pro Bein, ZWEI-WEGE): kein µ-Switch-Kontakt -> sanft nachsenken (Boden
            # suchen). Kontakt UND Rumpf über Soll (R3-gated via over_height) -> gesenktes Bein
            # sanft zurückholen, aber nie über die Basis hinaus (min 0). Ohne diesen Rückweg
            # ratscht dz bei kurzem Kontaktverlust nur nach unten und pumpt über alle Beine den
            # Rumpf hoch (Common-Mode); R3 als globaler Offset kann das nicht auflösen.
            td = self._foot_touch_down(leg)              # µ-Switch: berührt Boden von oben?
            self._g_contact[name] = td                   # Kontakt-Flag für Anzeige/Logger nachführen
            if not td:
                self._hold_dz[name] -= self.HOLD_SEEK_V
            elif over_height > 0.0:
                self._hold_dz[name] = min(0.0, self._hold_dz[name] + self.HOLD_SEEK_V)
            # obere Grenze +0.06: Fuß darf bis nahe/über Körperhöhe -> ein Bein auf einer hohen
            # Erhebung kann sich genug verkürzen, statt den Rumpf hochzudrücken (ST3215 anschlagfrei).
            z = _clamp(self.G_Z + self._hold_dz[name] - self.body_height_offset,
                       self.G_Z_FLOOR, 0.06)
            self._set_leg(leg, nx, ny, z)
    ...
    def spawn_random_bumps(self, amp_max, period_nom, seed=0, ramp_frames=240):
        ...
        for leg in self.legs:
            ...
            box_h = amp + 0.05; h2 = box_h/2.0; s2 = 0.2/2.0
            col = p.createCollisionShape(p.GEOM_BOX, halfExtents=[s2, s2, h2])
            vis = p.createVisualShape(p.GEOM_BOX, halfExtents=[s2, s2, h2], rgbaColor=[1,0.5,0,1])
...
def main():
    ...
    args = ap.parse_args()
    STAIR_RISE = args.rise; STAIR_RUN = args.run
