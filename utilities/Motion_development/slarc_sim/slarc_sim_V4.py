...
SLARC Simulation V4
===================
V3 + StairClimber: Automatische Treppensteige-Sequenz (N-Taste).
Inchworm-Muster: Körper anheben → Bein für Bein auf Stufe →
Körper nachziehen → normaler Gang.

Neue Tastenbelegung:
  N : Treppensteige-Sequenz starten / abbrechen
  X : Nur Körper anheben / absenken (Treppenmodus vorbereiten)
"""
...
class HexapodIK:
    ...
    def calculate_leg_angles(self, x, y, z):
        ...
        gamma = math.atan2(abs(z), L)   # immer positiv [0, pi/2]
        val_f = (self.L_F**2 + D**2 - self.L_T**2) / (2 * self.L_F * D)
        val_f = max(-1.0, min(1.0, val_f))
        alpha = math.acos(val_f)
        # Zwei IK-Äste je nach Fußhöhe:
        #   z <  0: Fuß unter Hüfte (Normal-/Standphase)
        #          theta_f = gamma - alpha
        #   z >= 0: Fuß über Hüfte (Hochschritt, Tuck)
        #          theta_f = -(alpha + gamma)
        # Stetig bei z=0 (beide Äste → -alpha wenn gamma=0)
        if z >= 0:
            theta_f = -(alpha + gamma)
        else:
            theta_f = gamma - alpha
...
class SlarcController:
    ...
    def update_gait(self):
        ...
        for leg in self.legs:
            ...
            # Nur Vorderbeine schwenken auf 90° seitlich —
            # Hinterbeine bleiben bei 45° (Vortrieb beim Hochschieben)
            if "front" in leg.name and t > 0.001:
                ...
                # Mittelbeine und Hinterbeine: Default-Position
        ...
        for leg in self.legs:
            if self.gait_mode == 4 and "front" in leg.name:
                ...
                # Kinematisches Limit: step_z darf Femur nicht über 90° bringen
                # Max: L_F = 175mm über Hüfte → step_z_max = 0.175 + 0.190 = 0.365m
                # Mit Sicherheitsabstand: 0.30m hard cap
                if step_z > 0.300:
                    step_z = 0.300
    ...
    def process_keyboard(self):
        ...
        for key, state in keys.items():
            if state & p.KEY_IS_DOWN:
                ...
                if key == ord('w'): self.body_height_offset = min(self.body_height_offset+0.002,  0.08)
                if key == ord('s'): self.body_height_offset = max(self.body_height_offset-0.002, -0.02)
            ...
            if state & p.KEY_WAS_TRIGGERED:
                ...
                if self.tuner.active:
                    ...
                    if key == ord('o'):
                        self.step_height = max(self.step_height - 0.008, 0.01)
                    ...
                    if key == ord('r'):
                        ...
                        self._stair_x_bias = 0.0
                    if key == ord('p'):
                        self.step_height = min(self.step_height + 0.008, 0.25)
                        print(f"Beinehöhe: {self.step_height*100:.1f} cm")
                    if key == ord('o'):
                        self.step_height = max(self.step_height - 0.008, 0.01)
                        print(f"Beinehöhe: {self.step_height*100:.1f} cm")
...
def main():
    ...
    print("\n=== SLARC V4 — Adaptives Terrain + StairClimber + Stereo ===")
    ...
    print(" T/G       : Arme heben/senken  F/H : vor/zurück")
    print(" Leertaste : Greifer Auf/Zu")
    print(" P / O     : Beinehöhe +/-0.8cm  (max 25cm → Femur 90° aufwärts)")
    print(" C         : Kamera-Debug an/aus")
    print(" R         : Reset (auch Treppen-Abbruch)")
    print(" ─────────────────────────────────────────────")
    print(" N         : Treppensteige-Sequenz starten/abbrechen")
    print("             Rollierendes Wellenmuster + Kontakt-Feedback")
    print(" ─────────────────────────────────────────────")
