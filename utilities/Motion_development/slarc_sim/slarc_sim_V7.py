...
class SlarcController:
    def __init__(self):
        ...
        # Säulen-Rig (Kontaktregelungs-Test): je 1 kinematische Säule unter jedem Fuß,
        #   fährt rhythmisch hoch/runter. Isoliert die Fuß-Boden-Suche vom Klettern.
        self.pillars_active = False; self.pillar_ids = []
        self.pillar_xy = []; self.pillar_t = 0.0
        self.pillars_frozen = False; self.pillar_static_h = []   # statisches Gelände
        self.PILLAR_BASE = 0.08    # Ruhe-Höhe der Säulen-Oberkante [m] (Headroom für ab)
        self.PILLAR_A    = 0.04    # Amplitude hoch/runter [m] (±4 cm)
        self.PILLAR_F    = 0.15    # Grundfrequenz [Hz] (phasenversetzt je Fuß)
        self.robot_id = None
        self.joint_map = {}
        self.cameras = None
        ...
        # ══════ v2-Kletterregelung (radikaler Neuanfang, stufenweise) ══════
        # P1 Pitch (Katze) nutzt die bestehende M7_PITCH_*-Schätzung weiter.
        self.V2_FEMUR_TARGET = -0.262  # EINSTELLBAR: Soll-Femurwinkel [rad] = „auf 15°,
        #   Tibia senkrecht". P2 regelt die Höhe so, dass der MITTLERE Femurwinkel der
        #   tragenden Beine hierher kommt → tiefer Körper, Femur auf, Tibia ⊥, max.
        #   Reichweiten-Budget. Mehr negativ = steiler auf = Körper tiefer.
        self.V2_RADIUS       = 0.229   # EINSTELLBAR: Stand-Radius [m] (Tibia⊥ bei 15°).
        #   Größer = Tibia splayt nach außen, kleiner = tuckt unter den Körper.
        self.V2_SPLAY        = 0.5     # Seitlich-Stellung [0..1]: blendet Vorder-/Hinterbeine
        #   von ±30°/±150° Richtung SEITLICH (±90°, „Tausendfüßler" statt „Spinne"). Grund:
        #   nur seitlich abstehende Beine treiben den Vorschub über die COXA; weit vorn macht
        #   es die Beinstreckung (Femur/Tibia) → Coxa-Blockadeerkennung wird blind. 0.5 = ±60°
        #   vorn / ±120° hinten, Mittelbeine bleiben bei ±90°.
        self.V2_H_KP         = 0.02    # P2-Höhenregler-Verstärkung (sanft, kein Bocken)
        self.V2_Z_FLOOR      = -0.30   # tiefster Fuß-z (max. Streckung) [m]
        self.V2_Z_INIT       = -0.12   # Anfangs-Stand-z vor dem Settle [m]
        self.V2_ENTER_RAMP   = 90      # Frames für sanfte Eintritts-Rampe (~0.4 s) —
        #   Ist-Pose → Gang-Pose interpoliert, statt zu snappen (sonst Kraftstoß/
        #   Katapult beim Moduswechsel, weil steife Servos die x-Staffelung in 1 Step rammen)
        self.V2_OFFSET_LIM   = 0.10    # max. gemeinsamer Höhen-Offset von P2 [m]
        # ── Stufe 2: Gehen (Vortrieb + event-getriebenes Nachsetzen) ──
        self.V2_BODY_V       = 0.0004  # Körper-Vortrieb je Frame [m] (entlang gepitchter x)
        self.V2_PM_SHIFT     = 0.0083  # Place-Move: Körper-Teilschritt je platziertem Bein [m]
                                       #   = V2_STEP_AHEAD/6 → Bein steht zur Re-Platzierung wieder neutral
        self.PM_CLIMB_PITCH_LIM = 0.45 # max. Kletter-Pitch-Sollwert [rad ~26°]
        self.PM_STEP_DETECT  = 0.025   # Stufen-Schwelle: Höhendiff vorn↔hinten der tragenden
                                       #   Füße [m], ab der erst Kletter-Pitch greift (Anti-Mitkopplung)
        # ── Unified-Gait (Mode 8): EIN Motion-Kern für alle Gangarten ──
        #   Phase pro Schwunggruppe; Z=Bogen(Phase), X=interpoliert, Y=Körperframe-Neutral.
        #   Kontakt (Last) übersteuert Phase: Fuß BLEIBT bei Aufsetzen. Coxa→höher tasten.
        self.UG_PHASE_RATE   = 0.020   # Phasenfortschritt je Frame (≈50 Frames/Schwung)
        self.UG_LAND_FORCE   = 3.0     # Aufsetzen erkannt, wenn Fußlast darüber [N] (robust gg. Flackern)
        self.UG_LAND_PHASE   = 0.55    # Last erst ab dieser Phase als Landung werten (erst nach Scheitel)
        self.UG_CFG = {
            # groups: gleichzeitig schwingende Beine | advance: Körper-Vortrieb-Modus | splay [0..1]
            'tripod': {
                'groups': [['front_left', 'mid_right', 'rear_left'],
                           ['front_right', 'mid_left', 'rear_right']],
                'advance': 'continuous',   # Körper rückt WÄHREND des Schwungs vor
                'splay':   0.0,
            },
            'placemove': {
                'groups': [['front_right'], ['mid_left'], ['rear_right'],
                           ['front_left'], ['mid_right'], ['rear_left']],
                'advance': 'when_planted', # Körper steht, rückt erst wenn Gruppe steht
                'splay':   self.V2_SPLAY,
            },
        }
        self._ug_name = 'tripod'       # aktive Gait-Konfig (Taste 8 zykliert)
        self._ug_ready = False
        self.ug_step_ahead = 0.05                        # Schrittlänge vor Neutral [m] (Taste 7; = V2_STEP_AHEAD)
        self.UG_STEP_OPTS  = [0.05, 0.08, 0.11, 0.14]    # wählbare Schrittlängen (Treppe: länger)
                                       #   → SHIFT abbrechen, Bein neu platzieren (kein Rückschub)
        self.V2_BODY_OFFSET  = 0.20    # Front-Gate: Körper überholt vordersten Fuß nicht
        self.V2_RESTEP_BACK  = 0.045   # EINSTELLBAR: Nachsetz-Auslöser — Fuß so weit hinter
        #   Neutral [m] → Bein hebt & setzt vor. Kleiner = häufiger, bessere Pose, weniger
        #   Drag/Überstreckung.
        self.V2_RESTEP_REACH = 0.27    # EINSTELLBAR: oder reach > diese [m] — proaktiv, gut
        #   unter dem IK-Limit 0.325, damit kein Bein in die Überstreckung schleift.
        self.V2_PITCH_KI     = 3.0     # v2-Pitch-Integral: Verstärkung gegen den Rest-Lean
        self.V2_PITCH_I_MAX  = 0.25    # Integral-Deckel [rad] (kein Windup)
        self.V2_ROLL_KI      = 3.0     # v2-Roll-Integral (gleicher Rest-Lean auf Roll-Achse)
        self.V2_ROLL_I_MAX   = 0.25    # Roll-Integral-Deckel [rad]
        self.V2_STEP_AHEAD   = 0.05    # Nachsetz-Ziel: so weit VOR Neutral aufsetzen [m]
        self.V2_LIFT         = 0.07    # Schwung-Hubhöhe über Standhöhe [m] (Riser-Clearance)
        self.V2_LIFT_MAX     = 0.16    # KLETTERN: max. Hub, bis zu dem der Riser-Tast den
        #   Schwungfuß progressiv höher hebt, wenn er die Stufenwand berührt (blindes
        #   Übersteigen). 0.16 deckt Stufen bis ~14 cm. Auf Flach inert (kein Kontakt im Schwung).
        self.V2_STEP_H_UP    = 0.020   # adaptiver Hub: rauf je BLOCKIERTEM Schwung [m]
        self.V2_STEP_H_DN    = 0.008   # adaptiver Hub: runter je FREIEM Schwung [m] —
        #   GEGENRICHTUNG: hören die Stufen auf (Block bleibt aus), klingt der Hub auf den
        #   Flach-Wert V2_LIFT zurück. Langsamer als rauf → kein Re-Block-Pendeln.
        self.V2_LIFT_V       = 0.012   # Hub-Geschw. [m/Frame] (kurzer Zyklus → wenig Drag)
        self.V2_REACH_V      = 0.010   # Vorschwung-Geschw. [m/Frame]
        self.V2_PROBE_V      = 0.0025  # Absenk-Geschw. bis Kontakt [m/Frame] — LANGSAM,
        #   sonst schlägt der Fuß in den Kontakt (Aufprallkraft ∝ Geschwindigkeit) →
        #   Absprung/Bouncing. 0.0025·240 ≈ 0.6 m/s = sanftes Aufsetzen.
        self.V2_CONTACT_ON   = 0.10    # Kontakt-Hysterese (Microswitch-treu): EIN ab dieser
        self.V2_CONTACT_OFF  = 0.02    # Kraft [N], AUS erst unter dieser (< ON). Dazwischen
        #   Zustand HALTEN → kein Blinken/Zittern um die Schaltschwelle (Ursache der
        #   Mikroruckler + grün/rot-Flackern, und des Such-/Re-Zentrier-Grenzzyklus).
        self.V2_TOUCH_DIST   = 0.002   # Berührungs-Schwelle [m]: Kontaktpunkt-Distanz ≤ 2 mm
        #   = echte Berührung (microswitch-treu), nicht der großzügige Collision-Margin.
        self.V2_DOWN_NZ      = 0.7     # Normalen-z-Grenze: >0.7 = waagerechte Fläche (Tritt,
        #   Sensor 1 „von oben"); <0.7 = senkrechte Wand (Riser, Sensor 2 Blockade). cos45°.
        self.V2_BLOCK_FORCE  = 0.5     # Kraft [N] für „festhängendes Bein" (Wand-Blockade,
        #   Sensor 2) — am echten Roboter die Servo-Last, nicht der Berührungs-Schalter.
        self.V2_BLOCK_COXA   = 0.5     # Coxa-Drehmoment-Schwelle [Nm] für Blockade im Vorholen.
        #   Isoliert gemessen: freier REACH-Schwung max 0.27, Blockade 1.3–2.5 → 0.5 trennt
        #   sauber und früh. Läuft NUR in REACH (schwingendes Bein), nie an geladenen
        #   Standbein-Coxas → keine Fehlauslösung auf Flach.
        self.V2_BLK_RETRACT  = 0.015   # Rückzug [m] des blockierten Fußes vor dem Höherheben.
        self.V2_STAIR_CLEAR  = 360     # Frames ohne Blockade, bis Schritthöhe wieder sinkt
        #   (= alle Beine über die Treppe). ~1.5 s bei 240 Hz, > ein Gang-Zyklus.
        self.V2_AIR_FRAMES   = 3       # Frames ohne Berührung, bis Kontakt auf AUS fällt
        #   (Release-Hysterese, microswitch-treu — verhindert Margin-Zappeln am Boden).
        self.V2_RECENTER_GAIN = 1.0    # Re-Zentrierung VOLL (jenseits des Totbands)
        self.V2_RECENTER_DEAD = 0.05   # TOTBAND [m] um Z_INIT: normales Einfedern (mm) und
        #   Geländevariation (±4 cm) werden NICHT korrigiert (sonst hebt die Korrektur den
        #   lasttragenden Fuß auf Marginal-Kontakt → reihum-Flackern). Nur echtes Ratschen
        #   jenseits ±5 cm (P2-Offset droht zu sättigen) wird zurückgeholt.
        self.V2_SEEK_V       = 0.0006  # eigene, LANGSAME Such-Rate (≠ Schwung-Aufsetzen):
        #   die Ein-Frame-Eindringung = Schlagkraft. 0.0006·15000 ≈ 9 N statt 37 N bei
        #   0.0025 → kein Fuß-pumpt-Körper-Prellen. Immer noch ~4× schneller als die Säulen.
        self.V2_SUPPORT_MIN  = 3       # ein Bein hebt nur, wenn ≥ so viele ANDERE tragen.
        #   3 = mind. Tripod-Stütz (statisch stabil). 4 fror auf der Treppe ein, weil
        #   dort nur 2–3 Füße tragen → kein Bein durfte je nachsetzen → Stranden.
        self.V2_REACH_EMERG  = 0.30    # NOTFALL-Nachsetzen: ist das hinterste Bein so weit
        #   überstreckt (nahe IK-Limit 0.325), darf es IMMER schwingen — Support-Gate egal.
        #   Stranden/Überstrecken ist schlimmer als kurz weniger Stützbeine.
        self.V2_SLOPE_DEAD   = 0.05    # Steigungs-Totband [rad ~3°]: |Steigung| darunter
        #   → pitch_sp=0 (Flach kein Pitch + Grenzkontakt-Rauschen raus). Erst echte
        #   Stufensteigung (>0.05) folgt er. Killt die Flach-Pitch-Mitkopplung.
        # ═══════════════════════════════════════════════════════════════════
    ...
    def _enter_place_move(self):
        """Modus 6 betreten: blinder Bein-Körper-Place-Move. Nutzt das v2-Setup
        (seitliche Neutralstellung, Eintritts-Rampe, Settle, Kontakt-Primitive)."""
        self._enter_v2()
        order = ['front_right', 'mid_left', 'rear_right',
                 'front_left', 'mid_right', 'rear_left']
        self._pm_order = [n for n in order if n in {l.name for l in self.legs}]
        self._pm_idx = 0
        self._pm_state = 'PLACE'        # PLACE = Bein setzen (Körper steht) | SHIFT = Körper vor
        self._pm_swing = 'LIFT'
        self._pm_shifted = 0.0
        self._pm_blocked_cycle = False  # war in diesem Zyklus ein Bein blockiert?
        self._pm_step_h = {l.name: self.V2_LIFT for l in self.legs}  # adaptiver Hub PRO BEIN
        pos, orn = p.getBasePositionAndOrientation(self.robot_id)
        self._pm_yaw_hold = p.getEulerFromQuaternion(orn)[2]          # Heading beim Eintritt
        self._pm_ready = True
        print("[Modus 6] Blinder Place-Move aktiv — ein Bein setzen (Körper steht), "
              "dann Körper nachrücken. Kein Bein wird je geschoben.")

    def _pm_foot_plane_pitch(self):
        """Neigung der Fußebene [rad] aus FK, NUR aus Füßen MIT Berührung (Sensor 1).
        Welt-z aus getLinkState (IST, körper-pitch-unabhängig). Vorne höher → >0
        (Aufstieg). Liefert None, wenn vorn ODER hinten kein tragendes Bein da ist."""
        ...
        def avg(group):
            pts = []
            for n in group:
                if n not in legmap or not self._v2_contact.get(n, False):
                    continue
                idx = self._foot_link_idx.get(n, -1)
                if idx < 0:
                    continue
                wp = p.getLinkState(self.robot_id, idx)[0]
                pts.append((wp[0], wp[2]))
            if not pts:
                return None
        ...
        if fa is None or ra is None:
            return None
        dz = fa[1] - ra[1]
        if abs(dz) < self.PM_STEP_DETECT:    # flach (kein echter Stufenversatz) → kein Pitch
            return None                      #   verhindert Mitkopplung auf dem Splay-Stand
        dx = fa[0] - ra[0]
        if abs(dx) < 1e-4:
            return None
        return math.atan2(dz, dx)

    def _place_move_gait(self):
        """Blinder statischer Kletterer: Bein–Körper–Bein–Körper. Pro Schritt wird EIN
        Bein platziert (LIFT→REACH mit Coxa-Blockade→LOWER bis Berührung), während der
        Körper STEHT (kein Schieben hängender Beine). Danach rückt der Körper eine
        Teil-Schrittweite vor (alle 6 Füße stehen → Schwerpunkt in der Stützfläche).
        Reine Reaktiv-Regelung, nur in der laufenden Sim/HW prüfbar."""
        if not getattr(self, '_pm_ready', False):
            self._enter_place_move()
        self._refresh_contact_cache()
        legmap = {leg.name: leg for leg in self.legs}

        # ── Eintritts-Rampe (wie _climb_v2) ──
        if getattr(self, '_v2_ramp', 0) > 0:
            self._v2_ramp -= 1
            a = 1.0 - self._v2_ramp / self.V2_ENTER_RAMP
            a = a * a * (3.0 - 2.0 * a)
            self._m7_pitch_eff = 0.0; self._m7_roll_eff = 0.0
            for leg in self.legs:
                s = self._v2_start[leg.name]; t = self._v2_target[leg.name]
                f = self._v2_foot[leg.name]
                f[0] = s[0] + (t[0] - s[0]) * a
                f[1] = s[1] + (t[1] - s[1]) * a
                f[2] = s[2] + (t[2] - s[2]) * a
                self._v2_base_z[leg.name] = f[2]
        ...
        # ── Settle: alle Füße runter bis Berührung ──
        if self._v2_settle > 0:
            self._v2_settle -= 1
            self._m7_pitch_eff = 0.0; self._m7_roll_eff = 0.0
            for leg in self.legs:
                f = self._v2_foot[leg.name]
                if self._foot_touch_down(leg):
                    self._v2_base_z[leg.name] = f[2]
                elif f[2] > self.V2_Z_FLOOR:
                    f[2] -= self.V2_PROBE_V
                self._m7_set_leg(leg, f[0], f[1], f[2])
            return

        # ── Kontakt je Fuß aus BERÜHRUNG (Sensor 1) + Release-Entprellung ──
        for leg in self.legs:
            if self._foot_touch_down(leg):
                self._v2_contact[leg.name] = True; self._v2_air[leg.name] = 0
            else:
                self._v2_air[leg.name] = self._v2_air.get(leg.name, 0) + 1
                if self._v2_air[leg.name] >= self.V2_AIR_FRAMES:
                    self._v2_contact[leg.name] = False

        # ── Kletter-Pitch-Sollwert aus der Fußebene (FK, kontaktgefiltert) ──
        #   1–2 Füße auf der nächsten Stufe → Steigung → Nase-hoch-Sollwert (Balancer).
        slope = self._pm_foot_plane_pitch()
        sp = 0.0
        if slope is not None and abs(slope) > self.V2_SLOPE_DEAD:
        ...
        self._m7_climb_pitch_sp = _clamp(self._m7_climb_pitch_sp,
                                         -self.PM_CLIMB_PITCH_LIM, self.PM_CLIMB_PITCH_LIM)
        climbing = abs(self._m7_climb_pitch_sp) > self.V2_SLOPE_DEAD

        # ── P2: Höhe über mittleren Femurwinkel der tragenden Beine ──
        #   Beim Klettern EINGEFROREN — sonst regelt P2 die 3D-Schub-Hebung wieder weg.
        fem = [p.getJointState(self.robot_id,
                               self.joint_map[f"{leg.name}_femur_joint"])[0]
               for leg in self.legs if self._v2_contact[leg.name]]
        if fem and not climbing:
            err = self.V2_FEMUR_TARGET - sum(fem) / len(fem)
            self._v2_offset = _clamp(self._v2_offset - err * self.V2_H_KP,
                                     -self.V2_OFFSET_LIM, self.V2_OFFSET_LIM)

        moving = abs(self.cmd_vel_x) > 1e-4
        cur = legmap[self._pm_order[self._pm_idx]]
        adv = 0.0

        # ── ZUSTANDSMASCHINE: PLACE (Bein setzen, Körper steht) / SHIFT (Körper vor) ──
        if moving and self._pm_state == 'PLACE':
            f = self._v2_foot[cur.name]; nx, ny = self._v2_neutral(cur)
            ph = self._pm_swing
            if ph == 'LIFT':
                f[2] += self.V2_LIFT_V
                top = self._v2_base_z[cur.name] + self._v2_offset + self._pm_step_h[cur.name]
                if f[2] >= top:
                    f[2] = top; self._pm_swing = 'REACH'
            elif ph == 'REACH':
                if self._coxa_torque(cur) > self.V2_BLOCK_COXA:    # echte Blockade (Coxa)
                    f[0] = max(nx, f[0] - self.V2_BLK_RETRACT)     # zurückziehen
                    self._pm_step_h[cur.name] = min(                # NUR dieses Bein höher
                        self._pm_step_h[cur.name] + self.V2_STEP_H_UP, self.V2_LIFT_MAX)
                    self._pm_swing = 'LIFT'                        # neu antasten
                    self._pm_blocked_cycle = True
                elif self._foot_touch_down(cur):                  # schon Boden → Fuß BLEIBT
                    self._v2_base_z[cur.name] = f[2]
                    self._pm_state = 'SHIFT'; self._pm_shifted = 0.0
                else:
                    tgt = nx + self.V2_STEP_AHEAD
                    if f[0] < tgt:
                        f[0] = min(tgt, f[0] + self.V2_REACH_V)
                    f[1] += (ny - f[1]) * 0.2
                    # Absenkender Bogen: Höhe an den horizontalen Fortschritt koppeln,
                    #   statt oben zu halten und danach langsam zu senken (kein Durch-
                    #   schwingen → kein Hochziehen). Kontakt beendet den Schwung sofort.
                    top  = self._v2_base_z[cur.name] + self._v2_offset + self._pm_step_h[cur.name]
                    land = self._v2_base_z[cur.name] + self._v2_offset
                    frac = _clamp((f[0] - nx) / self.V2_STEP_AHEAD, 0.0, 1.0)
                    f[2] = top + (land - top) * frac
                    if f[0] >= tgt - 1e-4:
                        self._pm_swing = 'LOWER'
            elif ph == 'LOWER':
                if self._foot_touch_down(cur) or f[2] <= self.V2_Z_FLOOR:
                    self._v2_base_z[cur.name] = f[2]               # Höhe erfasst, Fuß bleibt
                    self._pm_state = 'SHIFT'; self._pm_shifted = 0.0
                else:
                    f[2] -= self.V2_PROBE_V
            self._m7_set_leg(cur, f[0], f[1], f[2])
        elif moving and self._pm_state == 'SHIFT':
            adv = min(self.V2_BODY_V, self.V2_PM_SHIFT - self._pm_shifted)
            self._pm_shifted += adv
            if self._pm_shifted >= self.V2_PM_SHIFT - 1e-6:    # Teil-Schritt fertig
                self._pm_state = 'PLACE'; self._pm_swing = 'LIFT'
                self._pm_idx = (self._pm_idx + 1) % len(self._pm_order)
                if self._pm_idx == 0:                          # voller Zyklus vorbei
                    if not self._pm_blocked_cycle:             # keine Stufe → Hub sinkt
                        for n in self._pm_step_h:
                            self._pm_step_h[n] = max(
                                self._pm_step_h[n] - self.V2_STEP_H_DN, self.V2_LIFT)
                    self._pm_blocked_cycle = False

        # ── Standbeine: 3D-Vortrieb entlang GEPITCHTER Körperachse + Boden-Suche ──
        #   adv folgt der Nase-hoch-Lage → Horizontal- UND Vertikalkomponente:
        #   der Körper steigt die Stufe hoch statt nur horizontal anzudrücken.
        pe = self._m7_pitch_eff
        adv_x = adv * math.cos(pe)
        adv_z = adv * math.sin(pe) if climbing else 0.0   # Hebung NUR bei echtem Stufen-Pitch
        # Yaw-Drift-Ausgleich: Anfangskurs halten (IMU-Yaw), Q/E übersteuert.
        #   Gleiche getestete Konvention wie Mode 7 (Vorzeichen, Rotation um -dyaw).
        yaw_now = self._read_body_yaw()
        if abs(self.cmd_yaw) > 1e-4:
            self._pm_yaw_hold = yaw_now                # manuelles Lenken → neuer Sollkurs
            dyaw = self.cmd_yaw * self.M7_YAW_RATE
        else:
            yerr = math.atan2(math.sin(yaw_now - self._pm_yaw_hold),
                              math.cos(yaw_now - self._pm_yaw_hold))
            dyaw = _clamp(-self.M7_HEAD_KP * yerr, -self.M7_YAW_MAX, self.M7_YAW_MAX)
        cyaw = math.cos(-dyaw); syaw = math.sin(-dyaw)
        yawing = abs(dyaw) > 1e-6
        for leg in self.legs:
            if self._pm_state == 'PLACE' and leg.name == cur.name:
                continue                                           # platzierendes Bein separat
            f = self._v2_foot[leg.name]
            f[0] -= adv_x
            self._v2_base_z[leg.name] += adv_z
            if yawing:                                             # Drift-Ausgleich
                f[0], f[1] = f[0] * cyaw - f[1] * syaw, f[0] * syaw + f[1] * cyaw
            if not self._v2_contact[leg.name]:                     # luftleer → Boden suchen
                self._v2_base_z[leg.name] = max(
                    self._v2_base_z[leg.name] - self.V2_SEEK_V,
                    self.V2_Z_FLOOR - self._v2_offset)
            # W/S (body_height_offset) bleibt aktiv: höherer Körper → Fuß im Frame tiefer
            f[2] = _clamp(self._v2_base_z[leg.name] + self._v2_offset
                          - self.body_height_offset, self.V2_Z_FLOOR, -0.04)
            self._m7_set_leg(leg, f[0], f[1], f[2])

        # ── Skifahrer-Re-Zentrierung (Totband, nur tragende Füße) ──
        bzs_c = [self._v2_base_z[l.name] for l in self.legs if self._v2_contact[l.name]]
        if bzs_c:
            dev = sum(bzs_c) / len(bzs_c) - self.V2_Z_INIT
            if abs(dev) > self.V2_RECENTER_DEAD:
                corr = (dev - math.copysign(self.V2_RECENTER_DEAD, dev)) \
                       * self.V2_RECENTER_GAIN
                for l in self.legs:
                    if self._v2_contact[l.name]:
                        self._v2_base_z[l.name] -= corr

    # ════════════════════════════════════════════════════════════════════
    #  UNIFIED-GAIT (Mode 8): EIN Motion-Kern, parametriert per UG_CFG.
    #    Phase pro Schwunggruppe → X interpoliert, Z = Bogen(Phase),
    #    Y = Körperframe-Neutral (Splay/Quad gratis). Kontakt (LAST) übersteuert
    #    die Phase: Fuß bleibt beim Aufsetzen. Coxa → nur dieses Bein höher tasten.
    #    Körper-Vortrieb: 'continuous' (Tripod) oder 'when_planted' (Place-Move).
    #    Darüber: dieselbe Körperschicht (Pitch aus Fußebene, 3D-Schub, Yaw, W/S).
    # ════════════════════════════════════════════════════════════════════
    def _ug_arc_z(self, phase, start_z, top, land):
        """Schwungbogen: hebt start_z→top (Phase 0..½), senkt top→land (½..1), Smoothstep."""
        if phase <= 0.5:
            a = phase / 0.5; a = a * a * (3.0 - 2.0 * a)
            return start_z + (top - start_z) * a
        a = (phase - 0.5) / 0.5; a = a * a * (3.0 - 2.0 * a)
        return top + (land - top) * a

    def _enter_unified(self):
        cfg = self.UG_CFG[self._ug_name]
        self._ug_splay = cfg['splay']          # VOR _enter_v2: _v2_neutral liest das
        self._enter_v2()                       # Neutral (mit Splay), Ramp, Settle, v2-States
        self._ug_group = 0
        self._ug_phase = 0.0
        self._ug_sub = 'SWING'                 # SWING | SHIFT (nur when_planted)
        self._ug_shifted = 0.0
        self._ug_landed = {l.name: False for l in self.legs}
        self._ug_start  = {l.name: list(self._v2_foot[l.name]) for l in self.legs}
        self._ug_step_h = {l.name: self.V2_LIFT for l in self.legs}
        self._ug_blocked_cycle = False
        _, orn = p.getBasePositionAndOrientation(self.robot_id)
        self._ug_yaw_hold = p.getEulerFromQuaternion(orn)[2]
        self._ug_ready = True
        print("[Modus 8] Unified-Gait '%s' (%s). Pfeil↑ = laufen, Taste 8 = Gait wechseln."
              % (self._ug_name, cfg['advance']))

    def _ug_advance_group(self, cfg):
        """Schwunggruppe fertig → nächste Gruppe vorbereiten (+ Hub-Decay je Vollzyklus)."""
        self._ug_group = (self._ug_group + 1) % len(cfg['groups'])
        self._ug_phase = 0.0
        self._ug_sub = 'SWING'
        for name in cfg['groups'][self._ug_group]:
            self._ug_landed[name] = False
            self._ug_start[name] = list(self._v2_foot[name])     # aktuelle Pose = Schwung-Start
        if self._ug_group == 0:
            if not self._ug_blocked_cycle:
                for n in self._ug_step_h:
                    self._ug_step_h[n] = max(self._ug_step_h[n] - self.V2_STEP_H_DN, self.V2_LIFT)
            self._ug_blocked_cycle = False

    def _unified_gait(self):
        if not self._ug_ready:
            self._enter_unified()
        self._refresh_contact_cache()
        cfg = self.UG_CFG[self._ug_name]
        legmap = {l.name: l for l in self.legs}
        ...
        if getattr(self, '_v2_ramp', 0) > 0:
            ...
            for leg in self.legs:
                s = self._v2_start[leg.name]; t = self._v2_target[leg.name]
                f = self._v2_foot[leg.name]
                f[0] = s[0] + (t[0]-s[0])*a; f[1] = s[1] + (t[1]-s[1])*a
                f[2] = s[2] + (t[2]-s[2])*a
                self._v2_base_z[leg.name] = f[2]
                self._m7_set_leg(leg, f[0], f[1], f[2])
            return
        ...
        # ── Kontakt je Fuß aus Berührung + Release-Entprellung ──
        for leg in self.legs:
            if self._foot_touch_down(leg):
                self._v2_contact[leg.name] = True; self._v2_air[leg.name] = 0
            else:
        ...
        # ── Kletter-Pitch-Sollwert aus Fußebene (Stufen-Gate gegen Mitkopplung) ──
        slope = self._pm_foot_plane_pitch()
        sp = (self.M7_PITCH_SP_SIGN * slope
              if (slope is not None and abs(slope) > self.V2_SLOPE_DEAD) else 0.0)
        self._m7_climb_pitch_sp += (sp - self._m7_climb_pitch_sp) * self.M7_PITCH_LP
        self._m7_climb_pitch_sp = _clamp(self._m7_climb_pitch_sp,
                                         -self.PM_CLIMB_PITCH_LIM, self.PM_CLIMB_PITCH_LIM)
        climbing = abs(self._m7_climb_pitch_sp) > self.V2_SLOPE_DEAD

        # ── P2 Höhe (beim Klettern eingefroren) ──
        fem = [p.getJointState(self.robot_id, self.joint_map[f"{l.name}_femur_joint"])[0]
               for l in self.legs if self._v2_contact[l.name]]
        if fem and not climbing:
            err = self.V2_FEMUR_TARGET - sum(fem)/len(fem)
            self._v2_offset = _clamp(self._v2_offset - err*self.V2_H_KP,
                                     -self.V2_OFFSET_LIM, self.V2_OFFSET_LIM)
        ...
        moving = abs(self.cmd_vel_x) > 1e-4
        grp = cfg['groups'][self._ug_group]
        swing_set = set(grp) if self._ug_sub == 'SWING' else set()
        adv = 0.0

        # ── SCHWUNG der aktiven Gruppe (Last beendet, Coxa-Block → höher) ──
        if moving and self._ug_sub == 'SWING':
            self._ug_phase = min(1.0, self._ug_phase + self.UG_PHASE_RATE)
            ph = self._ug_phase
            all_done = True
            for name in grp:
                leg = legmap[name]; f = self._v2_foot[name]
                if self._ug_landed[name]:
                    continue
                nx, ny = self._ug_neutral(leg)
                sx, sy, sz = self._ug_start[name]
                tx = nx + self.ug_step_ahead
                if ph > 0.15 and self._coxa_torque(leg) > self.V2_BLOCK_COXA:  # Blockade
                    self._ug_step_h[name] = min(self._ug_step_h[name] + self.V2_STEP_H_UP,
                                                self.V2_LIFT_MAX)
                    self._ug_phase = max(0.15, self._ug_phase - 0.15)
                    self._ug_blocked_cycle = True
                    ph = self._ug_phase
                land = self._v2_base_z[name] + self._v2_offset
                top  = land + self._ug_step_h[name]
                f[0] = sx + (tx - sx) * ph
                f[1] = sy + (ny - sy) * ph
                f[2] = self._ug_arc_z(ph, sz, top, land)
                # Aufsetzen über LAST (robust gg. Distanz-Flackern), erst nach dem Scheitel
                if ph >= self.UG_LAND_PHASE and (self._foot_force(leg) > self.UG_LAND_FORCE
                                                 or self._foot_touch_down(leg)):
                    self._ug_landed[name] = True
                    self._v2_base_z[name] = f[2]
                self._m7_set_leg(leg, f[0], f[1], f[2])
                if not self._ug_landed[name]:
                    all_done = False
            if all_done or self._ug_phase >= 1.0:
                if cfg['advance'] == 'when_planted':
                    self._ug_sub = 'SHIFT'; self._ug_shifted = 0.0
                else:
                    self._ug_advance_group(cfg)

        elif moving and self._ug_sub == 'SHIFT':     # Place-Move: Körper rückt nach
            shift_tgt = self.ug_step_ahead / 6.0      # = Schrittlänge/6 → Bein landet wieder neutral
            adv = min(self.V2_BODY_V, shift_tgt - self._ug_shifted)
            self._ug_shifted += adv
            if self._ug_shifted >= shift_tgt - 1e-6:
                self._ug_advance_group(cfg)

        if cfg['advance'] == 'continuous' and moving:   # Körper rückt WÄHREND des Schwungs
            adv = self.V2_BODY_V

        # ── Stance-Beine: 3D-Schub entlang gepitchter Achse + Höhe + Yaw-Drift ──
        pe = self._m7_pitch_eff
        adv_x = adv * math.cos(pe)
        adv_z = adv * math.sin(pe) if climbing else 0.0
        yaw_now = self._read_body_yaw()
        if abs(self.cmd_yaw) > 1e-4:
            self._ug_yaw_hold = yaw_now; dyaw = self.cmd_yaw * self.M7_YAW_RATE
        else:
            yerr = math.atan2(math.sin(yaw_now - self._ug_yaw_hold),
                              math.cos(yaw_now - self._ug_yaw_hold))
            dyaw = _clamp(-self.M7_HEAD_KP * yerr, -self.M7_YAW_MAX, self.M7_YAW_MAX)
        cyaw = math.cos(-dyaw); syaw = math.sin(-dyaw); yawing = abs(dyaw) > 1e-6
        for leg in self.legs:
            if leg.name in swing_set and not self._ug_landed[leg.name]:
                continue                                 # aktiv schwingendes Bein: oben behandelt
            f = self._v2_foot[leg.name]
            f[0] -= adv_x
            self._v2_base_z[leg.name] += adv_z
            if yawing:
                f[0], f[1] = f[0]*cyaw - f[1]*syaw, f[0]*syaw + f[1]*cyaw
            if not self._v2_contact[leg.name]:
                self._v2_base_z[leg.name] = max(self._v2_base_z[leg.name] - self.V2_SEEK_V,
                                                self.V2_Z_FLOOR - self._v2_offset)
            f[2] = _clamp(self._v2_base_z[leg.name] + self._v2_offset
                          - self.body_height_offset, self.V2_Z_FLOOR, -0.04)
            self._m7_set_leg(leg, f[0], f[1], f[2])

        # ── Re-Zentrierung (Totband, nur tragende Füße) ──
        bzs_c = [self._v2_base_z[l.name] for l in self.legs if self._v2_contact[l.name]]
        if bzs_c:
            dev = sum(bzs_c)/len(bzs_c) - self.V2_Z_INIT
            if abs(dev) > self.V2_RECENTER_DEAD:
                corr = (dev - math.copysign(self.V2_RECENTER_DEAD, dev)) * self.V2_RECENTER_GAIN
                for l in self.legs:
                    if self._v2_contact[l.name]:
                        self._v2_base_z[l.name] -= corr

    def _ug_neutral(self, leg):
        """Körperframe-Neutral inkl. Splay (über _v2_neutral, Mode-8-Pfad)."""
        return self._v2_neutral(leg)
    ...
    def _foot_touch_down(self, leg):
        """Sensor 1 — BERÜHRUNG VON OBEN: Fuß steht auf einer (annähernd waagerechten)
        Fläche → Kontaktnormale SENKRECHT (nz > V2_DOWN_NZ). Das ist der Microswitch am
        Fußboden: misst Höhe / setzt auf / trägt Last. Eine seitliche WANDberührung
        (waagerechte Normale) zählt hier NICHT — die ist Sensor 2."""
        idx = self._foot_link_idx.get(leg.name, -1)
        if idx < 0:
            return False
        for c in self._contact_cache:
            if c[3] == idx and c[8] <= self.V2_TOUCH_DIST and c[7][2] > self.V2_DOWN_NZ:
                return True
        return False
    ...
    def _foot_blocked(self, leg):
        """Sensor 2 — HORIZONTALE BLOCKADE: Fuß drückt gegen eine (annähernd senkrechte)
        Wand → Kontaktnormale WAAGERECHT (|nz| klein) UND Kraft ≥ V2_BLOCK_FORCE. Das ist
        das festhängende Bein (Stufenwand getroffen / kommt nicht vor) → höher heben.
        Kraftbasiert, weil es um LAST geht (Servo-Last am echten Roboter), nicht Berührung."""
        idx = self._foot_link_idx.get(leg.name, -1)
        if idx < 0:
            return False
        for c in self._contact_cache:
            if c[3] == idx and abs(c[7][2]) < self.V2_DOWN_NZ and c[9] >= self.V2_BLOCK_FORCE:
                return True
        return False

    def update_contact_display(self):
        """Färbt jeden Fuß je Frame: GRÜN = tragender Kontakt (≥0.5 N),
        GELB = leichter Kontakt (<0.5 N), ROT = Luft. Klarer Bodenkontakt-Indikator."""
        self._refresh_contact_cache()                 # NACH stepSimulation → frisch
        ...
        for leg in self.legs:
            ...
            if idx < 0:
                continue
            # In Mode 7 den stabilen HYSTERESE-Kontakt nehmen (kein grün/rot-Flackern um
            #   die Schaltschwelle); sonst die rohe Kraft.
            hyst = getattr(self, '_v2_contact', None)
            if self.gait_mode == 7 and hyst is not None:
                col = (0.0, 0.9, 0.0, 1.0) if hyst.get(leg.name) else (0.9, 0.0, 0.0, 1.0)
            else:
                f = self._foot_force(leg)
                if   f >= 0.5:   col = (0.0, 0.9, 0.0, 1.0)   # grün: tragend
                elif f > 1e-3:   col = (1.0, 0.85, 0.0, 1.0)  # gelb: leichter Kontakt
                else:            col = (0.9, 0.0, 0.0, 1.0)    # rot: Luft
    ...
    def _v2_neutral(self, leg):
        # Neutralstellung Richtung SEITLICH geblendet (V2_SPLAY): die seitliche Richtung
        #   dieses Beins ist ±90° (copysign über mount_y). So treibt die COXA den Vorschub
        #   (hohe theta_c-Sensitivität) statt der Beinstreckung → Coxa-Blockadeerkennung
        #   funktioniert auch vorn. Mittelbeine (schon ±90°) bleiben unverändert.
        # Splay NUR im Place-Move-Klettern (Mode 6) bzw. nach Unified-Konfig (Mode 8).
        #   Mode 7 läuft kontinuierlich auf Flach → Original-Neutral, sonst degradiert der Gang.
        if self.gait_mode == 6:
            splay = self.V2_SPLAY
        elif self.gait_mode == 8:
            splay = getattr(self, '_ug_splay', 0.0)
        else:
            splay = 0.0
        lat = math.copysign(math.pi / 2.0, leg.mount_y)
        yaw = leg.mount_yaw + (lat - leg.mount_yaw) * splay
        return (leg.mount_x + self.V2_RADIUS * math.cos(yaw),
                leg.mount_y + self.V2_RADIUS * math.sin(yaw))

    def _v2_foot_world(self, leg):
        return p.getLinkState(self.robot_id, self._foot_link_idx[leg.name])[0]

    def _v2_w2b(self, W):
        """Welt-Punkt W → KOMMANDO-Körperframe (das, was _m7_set_leg erwartet).
        Schritt 1: W in den Basis-Frame (IST-Pose). Schritt 2: die Lage-Überlagerung
        (pitch_eff/roll_eff), die _m7_set_leg GLEICH WIEDER aufprägt, hier rausrechnen
        → Round-Trip konsistent. Auf Flach (pe=re=0) reine Basis-Transformation."""
        pos, orn = p.getBasePositionAndOrientation(self.robot_id)
        inv_p, inv_o = p.invertTransform(pos, orn)
        bf, _ = p.multiplyTransforms(inv_p, inv_o, list(W), [0, 0, 0, 1])
        pe = getattr(self, '_m7_pitch_eff', 0.0)
        re = getattr(self, '_m7_roll_eff', 0.0)
        cy, sy = math.cos(pe), math.sin(pe)
        cx, sx = math.cos(re), math.sin(re)
        bx, by, bz = bf
        # Inverse der Overlay-Rotation (erst Roll um x, dann Pitch um y zurück)
        fy = cx * by + sx * bz
        rz = -sx * by + cx * bz
        fx = cy * bx - sy * rz
        fz = sy * bx + cy * rz
    ...
    def _enter_v2(self):
        """Stand initialisieren — Füße GESTAFFELT entlang der Wellenordnung über den
        Schrittbereich verteilt, damit sie die Pose-Grenze NACHEINANDER erreichen
        (kein Kaltstart-Pile-up, der die hinteren Beine überstrecken lässt)."""
        legmap = {leg.name: leg for leg in self.legs}
        order = ['front_right', 'mid_left', 'rear_right',
                 'front_left', 'mid_right', 'rear_left']
        order = [n for n in order if n in legmap] or [l.name for l in self.legs]
        span = self.V2_STEP_AHEAD + self.V2_RESTEP_BACK
        self._v2_foot   = {}
        self._v2_base_z = {}                 # je Fuß: gemerkte Settle-Kontakthöhe
        self._v2_start  = {}                 # Eintritts-Rampe: Ist-Pose (Kommandoframe)
        self._v2_target = {}                 # Eintritts-Rampe: gestaffelte Ziel-Pose
        for i, name in enumerate(order):
            leg = legmap[name]
            nx, ny = self._v2_neutral(leg)
            # EBEN eintreten: alle Füße auf Neutral-x. Die frühere x-Staffelung war nur
            #   fürs Gehen (Anti-Pile-up) gedacht, erzeugt aber beim Stehen ungleiche
            #   Fußhöhen → nur ein Tripod trägt + diagonaler Kraftstoß beim Übergang.
            #   Gleichmäßiger 6-Fuß-Stand ist beim Eintritt wichtiger.
            cur = self._v2_w2b(self._v2_foot_world(leg))   # Ist-Pose (Kommandoframe)
            self._v2_start[name]  = [cur[0], cur[1], cur[2]]
            self._v2_target[name] = [nx, ny, self.V2_Z_INIT]
            self._v2_foot[name]   = [cur[0], cur[1], cur[2]]
            self._v2_base_z[name] = cur[2]
        self._v2_ramp = self.V2_ENTER_RAMP   # Eintritts-Rampe läuft
        self._v2_offset = 0.0                # gemeinsamer Höhen-Offset (P2)
        self._v2_pitch_i = 0.0               # Pitch-Integral (gegen Rest-Lean)
        self._v2_roll_i = 0.0                # Roll-Integral (gegen Rest-Lean)
        self._v2_settle = 240
        self._v2_swing_phase = {leg.name: None for leg in self.legs}
        self._v2_lift_top    = {leg.name: 0.0 for leg in self.legs}
        self._v2_lift_cap    = {leg.name: 0.0 for leg in self.legs}
        self._v2_step_h      = self.V2_LIFT                      # globaler adaptiver Hub
        self._v2_swing_blk   = {leg.name: False for leg in self.legs}  # Schwung blockiert?
        self._v2_blocked     = {leg.name: False for leg in self.legs}  # Bein in Blockade-Retry
        self._v2_block_age   = 9999                                    # Frames seit letzter Blockade
        self._v2_contact     = {leg.name: True for leg in self.legs}   # Hysterese-Kontakt
        self._v2_air         = {leg.name: 0 for leg in self.legs}       # Frames ohne Berührung
        self._m7_state  = {leg.name: 'STANCE' for leg in self.legs}  # für Steigungsfilter
        self._m7_climb_pitch_sp = 0.0
        self._v2_ready  = True

    def _climb_v2(self):
        """v2-Kletterregelung — STUFE 1: nur Körper-Schleifen auf gehaltenem Stand.
        P1: Körper parallel zur (kontaktgefilterten) Fußebene. P2: Höhe so, dass der
        mittlere Femurwinkel = V2_FEMUR_TARGET (tief, Femur auf, Tibia ⊥). Jeder Fuß
        hält seine eigene Settle-Kontakthöhe (Straddling bleibt), P2 verschiebt nur
        einen gemeinsamen Offset. KEIN Vortrieb, KEIN Stepping (das ist Stufe 2)."""
        if not getattr(self, '_v2_ready', False):
            self._enter_v2()
        self._refresh_contact_cache()
        legmap = {leg.name: leg for leg in self.legs}

        # ── EINTRITTS-RAMPE: Ist-Pose → gestaffelte Gang-Pose sanft interpolieren ──
        #   Verhindert den Kraftstoß/Katapult beim Moduswechsel: die steifen
        #   Positionsservos würden eine schlagartige x-Staffelung in EINEM Step rammen
        #   → Körper fliegt hoch und fällt zurück. Hier smoothstep über ~0.4 s, Körper
        #   sinkt kontrolliert von Stand- auf Gang-Höhe, Balance/P1 noch aus.
        if getattr(self, '_v2_ramp', 0) > 0:
            self._v2_ramp -= 1
            a = 1.0 - self._v2_ramp / self.V2_ENTER_RAMP   # 0→1
            a = a * a * (3.0 - 2.0 * a)                     # smoothstep
            self._m7_pitch_eff = 0.0
            self._m7_roll_eff  = 0.0
            for leg in self.legs:
                s = self._v2_start[leg.name]
                t = self._v2_target[leg.name]
                f = self._v2_foot[leg.name]
                f[0] = s[0] + (t[0] - s[0]) * a
                f[1] = s[1] + (t[1] - s[1]) * a
                f[2] = s[2] + (t[2] - s[2]) * a
        ...
        # ── P1: Körper parallel zur Fußebene (Katze) — mit Totband ──
        slope = self._m7_foot_plane_pitch(use_fk=True)     # kontaktgefiltert
        sp = 0.0
        if slope is not None and abs(slope) > self.V2_SLOPE_DEAD:
            sp = self.M7_PITCH_SP_SIGN * slope             # erst über Totband folgen
        self._m7_climb_pitch_sp += (sp - self._m7_climb_pitch_sp) * self.M7_PITCH_LP
        self._m7_climb_pitch_sp = _clamp(self._m7_climb_pitch_sp,
        ...
        if self.auto_balance:
            ...
            self.roll_bal, self.pitch_bal = self.balance.decay()
        # v2-INTEGRAL: der P-Balancer lässt unter dem konstanten Vortrieb-Moment
        # einen Rest-Lean stehen. Der I-Anteil fährt den Pitch auf den Sollwert
        # (0 auf Flach, Steigung auf der Treppe). Nur nach dem Settle, geclampt.
        if self._v2_settle <= 0:
            self._v2_pitch_i += ((pitch_m - self._m7_climb_pitch_sp)
                                 * self.V2_PITCH_KI / 240.0)
            self._v2_pitch_i = _clamp(self._v2_pitch_i,
                                      -self.V2_PITCH_I_MAX, self.V2_PITCH_I_MAX)
            self._v2_roll_i += (roll_m * self.V2_ROLL_KI / 240.0)   # roll_sp = 0
            self._v2_roll_i = _clamp(self._v2_roll_i,
                                     -self.V2_ROLL_I_MAX, self.V2_ROLL_I_MAX)
        else:
            self._v2_pitch_i = 0.0
            self._v2_roll_i = 0.0
        self._m7_pitch_eff = self.pitch_bal + self.pitch + self._v2_pitch_i
        self._m7_roll_eff  = self.roll_bal + self.roll + self._v2_roll_i

        # ── SETTLE: alle Füße runter bis Kontakt → Straddling-Stand etablieren ──
        if self._v2_settle > 0:
            self._v2_settle -= 1
            ...
            for leg in self.legs:
                f = self._v2_foot[leg.name]
                if self._foot_touch_down(leg):                   # Berührung, nicht Last
                    ncon += 1
                    self._v2_base_z[leg.name] = f[2]            # Kontakthöhe merken
                elif f[2] > self.V2_Z_FLOOR:
                    f[2] -= self.M7_PROBE_V                     # zum Boden ausfahren
                self._m7_set_leg(leg, f[0], f[1], f[2])
            if ncon >= 5 and self._v2_settle > 6:
                self._v2_settle = 6
            return

        # ── P2: mittleren Femurwinkel der tragenden Beine → V2_FEMUR_TARGET ──
        #   Tragende Beine über den STABILEN Hysterese-Kontakt wählen (nicht den rohen
        #   0.5-N-Kontakt): sonst wechselt die Menge bei jedem Mini-Preller → Offset
        #   zappelt → speist das Prellen mit.
        fem = []
        for leg in self.legs:
            if self._v2_contact[leg.name]:
                js = p.getJointState(self.robot_id,
                                     self.joint_map[f"{leg.name}_femur_joint"])
                fem.append(js[0])
        if fem:
            err = self.V2_FEMUR_TARGET - (sum(fem) / len(fem))
            self._v2_offset = _clamp(self._v2_offset - err * self.V2_H_KP,
        ...
        # ── Vortrieb (front-gated): nur so weit, bis vorderster Standfuß OFFSET hat ──
        #    SCHRITT 2: Ist IRGENDEIN Bein blockiert (Retry an der Stufenkante), wird der
        #    Vortrieb SOFORT gestoppt — sonst schiebt der Körper den festhängenden Fuß in
        #    die Kante (er „wird zurückgestoßen"). Erst wenn das Bein geklärt ist, geht's
        #    weiter (SCHRITT 7).
        adv = 0.0
        any_blocked = any(self._v2_blocked.values())
        if moving and not any_blocked:
            fxs = [self._v2_foot[leg.name][0]
                   for leg in self.legs if self._m7_state[leg.name] == 'STANCE']
            foremost = max(fxs) if fxs else self.V2_BODY_OFFSET
            adv = max(0.0, min(self.V2_BODY_V, foremost - self.V2_BODY_OFFSET))
        # SCHRITT (Schritthöhe-Restore): erst senken, wenn LANGE kein Bein mehr blockiert
        #   war (= alle Beine über die Treppe), nicht schon beim nächsten freien Schwung.
        self._v2_block_age += 1
        if (self._v2_block_age > self.V2_STAIR_CLEAR and not any_blocked
                and self._v2_step_h > self.V2_LIFT):
            self._v2_step_h = max(self._v2_step_h - self.V2_STEP_H_DN, self.V2_LIFT)

        # ── STANDFÜSSE: Körperframe-Vortrieb (bewährt, stabil) + P2-Höhe ──
        #    f[0] -= adv → Körper schiebt sich vor, Fuß bleibt (frictionAnchor) am
        #    Boden. Der FK-Halt (Ziel aus IST) kommt erst in Stufe 3 als Blockade-
        #    Erkennung dazu — auf Flach unnötig und (Lage-Mismatch) instabil.
        # Kontakt je Fuß aus BERÜHRUNG (microswitch-treu, nicht aus Last). EIN sofort bei
        #   Berührung; AUS erst nach V2_AIR_FRAMES Frames ohne Berührung (Release-Hysterese
        #   gegen Margin-Zappeln am Boden). Ein berührender, lastfreier Fuß bleibt „Kontakt"
        #   → keine Suche → keine Last-Rotation → kein Flackern.
        for leg in self.legs:
            if self._foot_touch_down(leg):
                self._v2_contact[leg.name] = True
                self._v2_air[leg.name] = 0
            else:
                self._v2_air[leg.name] = self._v2_air.get(leg.name, 0) + 1
                if self._v2_air[leg.name] >= self.V2_AIR_FRAMES:
                    self._v2_contact[leg.name] = False
        # Anzahl tragender Füße (aus dem stabilen Hysterese-Zustand, nicht der rohen Kraft)
        n_touch = sum(1 for l in self.legs if self._v2_contact[l.name])
        # --- Pass 1: Vortrieb + Boden-SUCHE (nur base_z anpassen) ---
        for leg in self.legs:
            if self._m7_state[leg.name] != 'STANCE':
                continue
            self._v2_foot[leg.name][0] -= adv
            # PRO-FUSS BODEN-SUCHE: Fuß ohne (Hysterese-)Kontakt zum Boden nachfahren.
            #   Den Wagenheber-Runaway verhindert die RE-ZENTRIERUNG (Mittelwert gepinnt →
            #   Körper kann nicht hochjacken), NICHT mehr ein Stütz-Gate. Daher nur noch:
            #   bei totalem Freiflug (0 Kontakt) Suche aus → Gravitation landet. Der
            #   Hysterese-Kontakt verhindert, dass ein leicht ge-lupfter Fuß sofort wieder
            #   sucht (kein Such-/Re-Zentrier-Grenzzyklus → keine Mikroruckler).
            if (not self._v2_contact[leg.name] and n_touch >= 1):
                self._v2_base_z[leg.name] = max(
                    self._v2_base_z[leg.name] - self.V2_SEEK_V,
                    self.V2_Z_FLOOR - self._v2_offset)
        # --- SKIFAHRER-RE-ZENTRIERUNG: base_z differenziell halten, Mittelwert konstant ---
        #   Das ist das fehlende EINfahren: fährt ein Bein in eine Senke aus (base_z runter),
        #   ziehen sich die anderen zurück, sodass die MITTLERE Beinlänge = Sollhöhe bleibt.
        #   Sonst ratscht base_z nur abwärts (Suche fährt aus, nichts fährt ein) → Anschlag.
        #   Höhe regelt P2 (offset), Lage der Balancer — base_z ist rein DIFFERENZIELL,
        #   damit die drei Schleifen orthogonal sind und nicht gegeneinander arbeiten.
        # --- SKIFAHRER-RE-ZENTRIERUNG: nur über die TRAGENDEN Füße ---
        #   Die belasteten Beine definieren die Körperhöhe; ihr Mittelwert wird auf die
        #   Sollhöhe gepinnt (kein Ratschen). Ein luftleeres, suchendes Bein (in einer
        #   Senke) wird AUSGESCHLOSSEN — sonst zieht sein Ausfahren den Mittelwert runter,
        #   die Korrektur hebt alle tragenden Füße an → sie lupfen ab → kollektiver
        #   Absink-Fang-Zyklus. Korrektur wird auch nur auf die tragenden Füße angewandt;
        #   die luftleeren greifen ungestört (volle Such-Rate) nach unten.
        bzs_c = [self._v2_base_z[l.name] for l in self.legs if self._v2_contact[l.name]]
        if bzs_c:
            dev = sum(bzs_c) / len(bzs_c) - self.V2_Z_INIT
            if abs(dev) > self.V2_RECENTER_DEAD:        # nur jenseits des Totbands
                corr = (dev - math.copysign(self.V2_RECENTER_DEAD, dev)) \
                       * self.V2_RECENTER_GAIN
                for l in self.legs:
                    if self._v2_contact[l.name]:
                        self._v2_base_z[l.name] -= corr
        # --- Pass 2: Fußziele setzen ---
        for leg in self.legs:
            if self._m7_state[leg.name] != 'STANCE':
                continue
            f = self._v2_foot[leg.name]
            f[2] = _clamp(self._v2_base_z[leg.name] + self._v2_offset,
                          self.V2_Z_FLOOR, -0.04)
            self._m7_set_leg(leg, f[0], f[1], f[2])

        # ── SCHWUNGBEINE: hoch (über Riser) → vor (unter Hüfte) → runter bis Kontakt ──
        for leg in self.legs:
            if self._m7_state[leg.name] != 'SWING':
                continue
            f = self._v2_foot[leg.name]
            nx, ny = self._v2_neutral(leg)
            ph = self._v2_swing_phase[leg.name]
            if ph == 'LIFT':
                f[2] += self.V2_LIFT_V
                if f[2] >= self._v2_lift_top[leg.name]:
                    f[2] = self._v2_lift_top[leg.name]
                    self._v2_swing_phase[leg.name] = 'REACH'
            elif ph == 'REACH':
                # SCHRITT 1: Blockade über COXA-Drehmoment erkennen — jeden Frame im Vorholen.
                #   Die Coxa treibt den Vorschub last-/momentfrei; steigt ihr Moment, drückt
                #   der Fuß gegen die Stufenkante. Immun gegen vertikale Aufprall-Stöße.
                if self._coxa_torque(leg) > self.V2_BLOCK_COXA:
                    self._v2_blocked[leg.name] = True      # → Vortrieb stoppt (Schritt 2)
                    self._v2_block_age = 0
                    # SCHRITT 3: blockiertes Bein ZURÜCKZIEHEN + Schritthöhe VERGRÖSSERN
                    f[0] = max(nx, f[0] - self.V2_BLK_RETRACT)
                    self._v2_step_h = min(self._v2_step_h + self.V2_STEP_H_UP,
                                          self.V2_LIFT_MAX)
                    self._v2_lift_top[leg.name] = (self._v2_base_z[leg.name]
                                                   + self._v2_offset + self._v2_step_h)
                    self._v2_swing_phase[leg.name] = 'LIFT'   # SCHRITT 4: neu antasten (höher)
                    self._v2_swing_blk[leg.name] = True
                else:
                    # SCHRITT 5: im Vorholen NICHT blockiert → Fuß ist über der Kante
                    tgt = nx + self.V2_STEP_AHEAD
                    if f[0] < tgt:
                        f[0] = min(tgt, f[0] + self.V2_REACH_V)
                    f[1] += (ny - f[1]) * 0.2
                    if f[0] >= tgt - 1e-4:
                        self._v2_swing_phase[leg.name] = 'LOWER'   # → absenken, Höhe messen
            elif ph == 'LOWER':
                # SCHRITT 5/6: absenken bis BERÜHRUNG VON OBEN (Sensor 1) → Stufenhöhe erfasst.
                if self._foot_touch_down(leg) or f[2] <= self.V2_Z_FLOOR:
                    self._m7_state[leg.name] = 'STANCE'
                    self._v2_base_z[leg.name] = f[2]            # erfasste Höhe = neue Referenz
                    self._v2_swing_phase[leg.name] = None
                    self._v2_blocked[leg.name] = False          # Bein geklärt → Vortrieb frei (7)
                    self._v2_swing_blk[leg.name] = False
                    # Schritthöhe wird NICHT hier gesenkt — bleibt bis alle Beine über die
                    #   Treppe sind (Restore oben, gated über _v2_block_age).
                else:
                    f[2] -= self.V2_PROBE_V
            self._m7_set_leg(leg, f[0], f[1], f[2])

        # ── EVENT-NACHSETZEN: hinterstes Bein über Pose-Grenze/Überstreckung hebt,
        #    wenn ≥ V2_SUPPORT_MIN ANDERE tragen und keins gerade schwingt ──
        if moving:
            swinging = any(self._m7_state[l.name] == 'SWING' for l in self.legs)
            if not swinging:
                cands = []
                for leg in self.legs:
                    if self._m7_state[leg.name] != 'STANCE':
                        continue
                    a = self._v2_foot[leg.name]
                    nx, ny = self._v2_neutral(leg)
                    back  = nx - a[0]
                    reach = math.hypot(a[0] - leg.mount_x, a[1] - leg.mount_y)
                    airborne = not self._v2_contact[leg.name]   # Berührung statt Last
                    # Hängendes Standbein ist Kandidat (sonst Deadlock), aber mit
                    # NORMALER Priorität (=back) — kein Vordrängeln, sonst hoggen die
                    # Vorderbeine (Dauerschwingen) und die hinteren kommen nie dran.
                    if airborne or back > self.V2_RESTEP_BACK or reach > self.V2_RESTEP_REACH:
                        cands.append((back, leg))
                if cands:
                    cands.sort(key=lambda c: c[0], reverse=True)   # hinterstes zuerst
                    leg = cands[0][1]
                    a = self._v2_foot[leg.name]
                    leg_reach = math.hypot(a[0] - leg.mount_x, a[1] - leg.mount_y)
                    # Airborne-Override: ein bereits abgehobenes (überstrecktes) Bein
                    # trägt eh nicht → darf IMMER nachsetzen (bricht jeden Deadlock).
                    # NOTFALL-Override: auch ein noch (grade so) tragendes, aber stark
                    # überstrecktes Bein darf IMMER — sonst strandet es auf der Treppe.
                    leg_air = not self._v2_contact[leg.name]    # Berührung statt Last
                    emergency = leg_air or leg_reach > self.V2_REACH_EMERG
                    others = sum(1 for l in self.legs
                                 if l.name != leg.name and self._v2_contact[l.name])
                    if emergency or others >= self.V2_SUPPORT_MIN:
                        self._m7_state[leg.name] = 'SWING'
                        self._v2_swing_phase[leg.name] = 'LIFT'
                        self._v2_swing_blk[leg.name] = False
                        self._v2_lift_top[leg.name] = (self._v2_base_z[leg.name]
                                                       + self._v2_offset + self._v2_step_h)
                        # Riser-Tast-Obergrenze für progressives Höherheben in REACH
                        self._v2_lift_cap[leg.name] = (self._v2_base_z[leg.name]
                                                       + self._v2_offset + self.V2_LIFT_MAX)
    ...
    def _feeler_gait(self):
        ...
        if not moving:
            ...
            back_lim = -2.5 * self.M7_STRIDE            # Sicherheits-Rücklage
            ...
            adv = max(0.0, min(self.M7_BODY_V, foremost - self.M7_BODY_OFFSET))
    ...
    def update_gait(self):
        ...
        # Modus 7: v2-Kletterregelung (radikaler Neuanfang) — Stufe 1
        if self.gait_mode == 7:
            self._climb_v2()
            return
    ...
    def toggle_pillars(self):
        """Säulen-Rig (Kontaktregelungs-Test, Taste 0). ZYKLUS auf Taste 0 (Satzzeichen-
        Tasten sind auf nativem Windows unzuverlässig): AUS → BEWEGT (Sinus) → EINGEFROREN
        (statische Zufallshöhen) → AUS. Statisch = realistischer Boden (bewegt sich nicht).
        Erneutes Durchschalten würfelt beim Einfrieren neue Höhen."""
        if not self.pillars_active:
            H0 = self.PILLAR_BASE
            half = [0.06, 0.06, 0.25]
            col = p.createCollisionShape(p.GEOM_BOX, halfExtents=half)
            self.pillar_ids = []; self.pillar_xy = []
            for leg in self.legs:
                idx = self._foot_link_idx.get(leg.name, -1)
                if idx < 0:
                    continue
                ...
                vis = p.createVisualShape(p.GEOM_BOX, halfExtents=half,
                                          rgbaColor=[0.30, 0.55, 0.85, 1.0])
                pid = p.createMultiBody(0, col, vis, [w[0], w[1], cz])
                _set_contact(pid)
                self.pillar_ids.append(pid)
                self.pillar_xy.append((w[0], w[1]))
            pos, orn = p.getBasePositionAndOrientation(self.robot_id)
            p.resetBasePositionAndOrientation(
                self.robot_id, [pos[0], pos[1], pos[2] + H0], orn)
            self.pillar_t = 0.0; self.pillars_active = True
            self.pillars_frozen = False; self.pillar_static_h = []
            self.auto_balance = True; self.balance.reset()
            print("Säulen: AN (BEWEGT/Sinus). 0 erneut = einfrieren (statisch).")
            return
        if not self.pillars_frozen:
            import random as _rnd
            lo = self.PILLAR_BASE - self.PILLAR_A
            hi = self.PILLAR_BASE + self.PILLAR_A
            self.pillar_static_h = [_rnd.uniform(lo, hi) for _ in self.pillar_ids]
            self.pillars_frozen = True
            print(f"Säulen: EINGEFROREN — statische Zufallshöhen "
                  f"[{lo*100:.0f}–{hi*100:.0f} cm]. 0 erneut = aus.")
            return
        for pid in self.pillar_ids:
            p.removeBody(pid)
        self.pillar_ids = []; self.pillar_xy = []
        self.pillars_active = False; self.pillars_frozen = False
        self.pillar_static_h = []
        # SANFT absetzen: Roboter so weit senken, dass der tiefste Fuß auf z=0 (Boden)
        #   liegt — sonst Freifall von den Säulen (8 cm) mit hartem Aufschlag + Austasten.
        fzs = [p.getLinkState(self.robot_id, self._foot_link_idx[l.name])[0][2]
               for l in self.legs if l.name in self._foot_link_idx]
        if fzs:
            pos, orn = p.getBasePositionAndOrientation(self.robot_id)
            p.resetBasePositionAndOrientation(
                self.robot_id, [pos[0], pos[1], pos[2] - min(fzs)], orn)
        print("Säulen: AUS (sanft auf den Boden abgesetzt)")

    def update_pillars(self):
        if not self.pillars_active:
            return
        if self.pillars_frozen:
            # STATISCHES Gelände: Boden bewegt sich nicht (realistischer Fall).
            #   Mit gewürfelten Höhen → diese setzen; sonst aktuelle Höhen halten.
            if self.pillar_static_h:
                for i, pid in enumerate(self.pillar_ids):
                    x, y = self.pillar_xy[i]
                    p.resetBasePositionAndOrientation(
                        pid, [x, y, self.pillar_static_h[i] - 0.25], [0, 0, 0, 1])
            return
        self.pillar_t += 1.0 / 240.0
        for i, pid in enumerate(self.pillar_ids):
            x, y = self.pillar_xy[i]
            # phasenversetzte Sinus, je Fuß leicht andere Frequenz → aperiodisch,
            #   erzeugt zugleich kippende Fußebene (testet P1-Pitch) + reine Höhe (Boden-Suche)
            f  = self.PILLAR_F * (1.0 + 0.13 * i)
            ph = i * (2.0 * math.pi / 6.0)
            top = self.PILLAR_BASE + self.PILLAR_A * math.sin(
                2.0 * math.pi * f * self.pillar_t + ph)
            p.resetBasePositionAndOrientation(pid, [x, y, top - 0.25], [0, 0, 0, 1])
        ...
        for key, state in keys.items():
            ...
            if state & p.KEY_WAS_TRIGGERED:
                ...
                if self.tuner.active:
                    ...
                    if key == ord('6'):
                        self.gait_mode = 6
                        self._pm_ready = False   # Place-Move beim ersten Frame initialisieren
                        for g in self._stall.values(): g.reset()
                        print("Modus 6: TREPPE — blinder Place-Move (Bein–Körper–Bein). "
                              "Pfeil↑ = klettern. Körper schiebt nie ein hängendes Bein.")
                    if key == ord('7'):
                        opts = self.UG_STEP_OPTS
                        nxt = min((o for o in opts if o > self.ug_step_ahead + 1e-6),
                                  default=opts[0])      # zyklisch: nächstgrößer, sonst kleinster
                        self.ug_step_ahead = nxt
                        print("Schrittlänge: %.0f cm (Treppe: länger, damit der Fuß auf "
                              "die Trittfläche statt auf die Kante kommt)" % (nxt * 100))
                    if key == ord('8'):
                        names = list(self.UG_CFG.keys())
                        if self.gait_mode == 8:                  # schon aktiv → Gait wechseln
                            i = names.index(self._ug_name)
                            self._ug_name = names[(i + 1) % len(names)]
                        self.gait_mode = 8
                        self._ug_ready = False                   # beim ersten Frame initialisieren
                        for g in self._stall.values(): g.reset()
                        print("Modus 8: UNIFIED-GAIT '%s' — ein Motion-Kern für alle "
                              "Gangarten. Pfeil↑ = laufen, Taste 8 erneut = Gait wechseln."
                              % self._ug_name)
                    if key == ord('9'):
                        self.gait_mode = 7
                        self._v2_ready = False   # v2-Kletterregelung beim ersten Frame initialisieren
                        for g in self._stall.values(): g.reset()
...
class MotionLogger:
    ...
    def tick(self):
        ...
        for n in self._legs():
            ...
            if state == 'SWING':
                ph = getattr(c, '_v2_swing_phase', {}).get(n)
                # Phase OHNE 'SWING:'-Präfix → kein führendes S mehr (STANCE bleibt der
                #   einzige S-Zustand, kein Doppel-S beim Filtern/Plotten nach 1. Buchstabe).
                state = ph if ph else 'SWING'
...
def main():
    ...
    robot.init_pybullet(build_stairs=True, spawn_can=False)
