...
# Sentinel for check_expected: asserts a field must NOT be present
MUST_BE_ABSENT = object()
...
TEST_CASES = [
    ...
    # --- Named locations (continued) ---
    {
        "id":       "LOC-04",
        "group":    "Named locations",
        "intention":"Move to the named area north_garden and map all unknown cells there.",
        "expected": {"location_name": "north_garden",
    ...
    },
    {
        "id":       "LOC-06",
        "group":    "Named locations",
        "intention":"The inspection point is called anomaly_pos -> go there and inspect the object.",
    ...
    # --- Charging ---
    {
        "id":       "CHG-01",
        "group":    "Charging",
        "intention":"Return home when battery drops below 30% and charge to 80% before continuing.",
        "expected": {"stop_conditions": ["battery_threshold"], "battery_threshold": 30,
                     "return_home_on_stop": True, "charge_to": 80},
    },
    {
        "id":       "CHG-02",
        "group":    "Charging",
        "intention":"Explore southward and return home when battery is below 40%.",
        "expected": {"direction_bias": "south", "battery_threshold": 40,
                     "return_home_on_stop": True, "charge_to": 90},
        "note":     "charge_to: 90 added by normalize_policy heuristic",
    },
    {
        "id":       "CHG-03",
        "group":    "Charging",
        "intention":"Explore for 10 minutes then return home.",
        "expected": {"stop_conditions": ["time_elapsed"], "time_limit_seconds": 600,
                     "return_home_on_stop": True, "charge_to": MUST_BE_ABSENT},
        "note":     "time-based return — charge_to must NOT be set",
    },
    {
        "id":       "CHG-04",
        "group":    "Charging",
        "intention":"Greet the cat when battery drops below 50%, return home and charge to 90%.",
        "expected": {"object_name": "cat", "object_interaction": "greet",
                     "battery_threshold": 50, "return_home_on_stop": True, "charge_to": 90},
    },
    {
        "id":       "CHG-05",
        "group":    "Charging",
        "intention":"Head north and return home when battery is below 25%, then charge fully.",
        "expected": {"direction_bias": "north", "battery_threshold": 25,
                     "return_home_on_stop": True, "charge_to": 100},
    },
    ...
    # --- Validator edge cases ---
    {
        "id":       "VAL-01",
        "group":    "Validator",
        "intention":"Follow the cat westward.",
        "expected": {},
        "note":     "follow with no termination -> validator must warn",
        "expect_warnings": True,
    },
    {
        "id":       "VAL-02",
        "group":    "Validator",
        "intention":"Follow the cat westward for 5 minutes.",
        "expected": {"object_name": "cat", "object_interaction": "follow",
                     "time_limit_seconds": 300},
        "note":     "follow + time_elapsed -> validator must NOT warn",
...
# Handles nested list equality, Ellipsis wildcard, MUST_BE_ABSENT.
# ---------------------------------------------------------------
def check_expected(policy: dict, expected: dict) -> list[str]:
    failures = []
    for field, exp_val in expected.items():
        if exp_val is MUST_BE_ABSENT:
            if field in policy and policy[field] is not None:
                failures.append(
                    f"'{field}' must be absent/null, got {repr(policy[field])}")
            continue
        ...
        if exp_val is ...:
            ...
            if actual != exp_val:
                failures.append(
                    f"'{field}': expected {repr(exp_val)}, got {repr(actual)}")
    return failures
...
def run_test_suite():
    ...
    for tc in TEST_CASES:
        ...
        try:
            ...
            if tc.get("expect_warnings", False):
                passed = len(exp_fail) == 0 and len(val_warn) > 0
            else:
