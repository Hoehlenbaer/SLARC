...
MODELS = {
    ...
    'combined': {
        ...
        'outputs': ['disp_final', 'normals_s1', 'disp_s8', 'seg',
...
def generate_calib_data(model_key, model_cfg):
    ...
    if model_key == 'backbone':
        ...
        # Combined braucht: Backbone-Features L (alle 4), Backbone-Features R (s4, s8), img_l
        bb_har = MODELS['backbone']['har']
        if os.path.exists(bb_har):
            print("   🔄 Generiere Backbone-Features (L+R) für Combined...")
            bb_runner = ClientRunner(har=bb_har, hw_arch='hailo8')
            bb_hn = bb_runner.get_hn_dict()
            bb_all = list(bb_hn.get("layers", {}).keys())
            bb_input = sorted([l for l in bb_all if 'input_layer' in l.lower()])[0]
            ...
            try:
                print("       → Backbone(left)...")
                features_l = run_backbone_batch(calib_l[:n])
                print("       → Backbone(right)...")
                features_r = run_backbone_batch(calib_r[:n])
                
                print(f"   ✅ {n} Feature-Paare generiert")
