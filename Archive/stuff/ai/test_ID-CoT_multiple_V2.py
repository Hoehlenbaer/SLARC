...
- ID: PATH_C  | Type: PATH | width: 15""",
    ...
    },
    {
        ...
        "Data": """- ID: PATH_1 | Type: PATH | width: 90
...
def display_results(results):
    ...
    total_time = sum(r.get('Inference Time (s)', 0) for r in results if r.get('Result') != 'ERROR')
    valid_tests = len([r for r in results if r.get('Result') != 'ERROR' and 'Inference Time (s)' in r])
    
    avg_time = total_time / valid_tests if valid_tests > 0 else 0
    
    print("\n" + "="*80)
    print(f"{'TEST NAME':<30} | {'EXPECTED':<20} | {'ACTUAL':<20} | {'TIME (s)':<10} | {'STATUS'}") # Column for time added
    print("-" * 80)
    ...
    print(f"Average Inference Time per Test: {avg_time:.4f}s") # Average time added
    print("="*80)
