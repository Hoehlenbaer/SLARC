...
### HIERARCHY:
1. explore     -> This is needed to know the environment -> Environment_Mapped 
2. find cup    -> This is needed to know the exact location of the cup -> Target_Visible
3. move cup    -> This is needed to bridge the distance to the cup and get in grasping range -> In_Grasping_Range (Target_Visible=T AND Distance < 25cm)
4. grab cup    -> This is needed to grab the cup with the gripper -> Object_In_Gripper (In_Grasping_Range=T)
5. move home   -> This is needed to get back home to the location, where the cup shall be placed -> Robot_At_Home     (Object_In_Gripper=T)
6. place cup   -> This is needed to reach the mission goal -> Mission_Complete  (Robot_At_Home=T)
